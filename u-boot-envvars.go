...
bootargs=console=ttymxc0,115200n8 mem=1024m quiet root=/dev/ram0 rootfstype=ext4 initrd=0x89000000,3M rootwait rw init=/init start ip=192.168.101.100::192.168.101.1:255.255.255.0::eth0:on
bootcmd=run readmac;sf probe 0;sf read ${fdt_addr} 0x00080000 ${sz_fdt};sf read ${loadaddr} 0x00100000 ${sz_kernel};sf read ${initrd_addr} 0x00300000 ${sz_initrd};bootz ${loadaddr} - ${fdt_addr}
bootdelay=3
